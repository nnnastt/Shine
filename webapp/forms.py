...
from webapp.models import UserInfo, PaymentCard, UserAddress, Order
...
class AdminPaymentCardForm(forms.ModelForm):
    card_holder = forms.CharField(
        label='Имя владельца',
        widget=forms.TextInput(attrs={
            "class": "form-control",
...
class OrderStatusForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.fields['tracking_number'] = forms.CharField(
            required=False,
            widget=forms.TextInput(attrs={
                "class": "form-control",
    ...
    def clean(self):
        ...
        if status == 'cancelled' and not reason:
            raise ValidationError('При отмене заказа необходимо указать причину')
